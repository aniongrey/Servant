...
describe('POE2EventParser', () => {
  ...
  it('parses death and level events', () => {
    expect(
      parser.parseLine('2026/08/31 20:12:01 123 INFO Client 123 [INFO Client] Shiro has been slain.')
    ).toMatchObject({
      type: 'PLAYER_DEATH',
      player: 'Shiro'
    });
    expect(
      parser.parseLine('2026/08/31 20:12:02 123 INFO Client 123 [INFO Client] Shiro is now level 42')
    ).toMatchObject({
